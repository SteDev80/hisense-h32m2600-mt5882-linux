...
mt53xx_timer_interrupt(int irq, void *dev_id)
{
	...
	/* Diagnostic fallback: stop the storm after the first Timer0 event. */
	timer_ctrl = __bim_readl(REG_RW_TIMER_CTRL);
	timer_ctrl &= ~(TCTL_T0EN | TCTL_T0AL);
	__bim_writel(timer_ctrl, REG_RW_TIMER_CTRL);
	/* Flush the BIM write-one-to-clear before GIC unmasks the level IRQ. */
	__bim_writel(1U << (irq - SPI_OFFSET), REG_RW_IRQCLR);
	(void)__bim_readl(REG_RO_IRQST);
#endif
