...
$logPath = 'C:\Users\steki\Documents\Codex\2026-08-30\referenced-chatgpt-conversation-this-is-an\outputs\buildroot-h32m2600\uart-original-initramfs-boot.txt'
$all = ''
...
try {
    ...
    $initrd = Run-Uboot 'fatload usb 0:1 0x04000000 uInitrd-h32m2600-buildroot' 120
    if ($initrd -notmatch '12597127 bytes read') { throw 'Initramfs VNC non caricato integralmente' }
    ...
    if ($initrdInfo -notmatch 'Verifying Checksum \.\.\. OK') { throw 'CRC initramfs non valido' }

    [void](Run-Uboot ("setenv bootargs " + $bootargs) 30)
    $env = Run-Uboot 'printenv bootargs' 30
    if ($env -notmatch 'rdinit=/init') { throw 'bootargs non impostati correttamente' }
    ...
    while ([DateTime]::UtcNow -lt $deadline) {
        ...
        if ($text) {
            ...
            if ($all -match '=== MT5882 BUILDROOT RESCUE READY ===') {
                [Console]::WriteLine("`r`n[HOST] BUILDROOT_RESCUE_READY")
                break
            }
        }
        Start-Sleep -Milliseconds 20
